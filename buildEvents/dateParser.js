...
export const beforeActivityDateParser = {
  ...
  parseEventDate: (description, actStartDate, actEndDate) => {
    ...
    nums.length === 2
      ? eventEnd.setDate(eventEnd.getDate() - nums[1])
      : eventEnd.setDate(eventEnd.getDate() - nums[0]);
...
export const duringActivityDateParser = {
  ...
  parseEventDate: (description, actStartDate, actEndDate) => {
    ...
    if (dayStart !== -1 && dayEnd !== -1 && dayEnd > dayStart) {
      let daySub =
        duringActivityDateParser.wordMap[
          description.substring(dayStart, dayEnd)
        ];
    ...
    if (description.includes("活動期間")) {
      ...
      if (description.includes("1周內")) {
        eventEnd.setDate(
          actEndDate.getDate() + 1 + ((7 - actEndDate.getDay() - 1) % 7)
        );
      } else if (description.includes("的周日")) {
        eventStart.setDate(
          actStartDate.getDate() + ((7 - actStartDate.getDay()) % 7)
        );
...
const parseEventTime = (timeStr) => {
  ...
  const endSuffix =
    endTime.includes("下午") ||
    (!endTime.includes("下午") && startTime.includes("下午"))
      ? "PM"
      : "AM";
...
const formatDate = (eventStart, eventEnd) => {
  return {
    start: eventStart.toLocaleString("en-US", {
      month: "2-digit",
      day: "2-digit",
      year: "numeric",
    }),
    end: eventEnd.toLocaleString("en-US", {
      month: "2-digit",
      day: "2-digit",
      year: "numeric",
    }),
...
const formatDateTime = (eventStart, eventEnd) => {
  ...
  return {
    start: eventStart.toLocaleString("en-US", options),
